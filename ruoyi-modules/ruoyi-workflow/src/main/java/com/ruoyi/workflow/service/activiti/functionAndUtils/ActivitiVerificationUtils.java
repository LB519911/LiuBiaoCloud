...
public class ActivitiVerificationUtils {
    ...
    public static ProcessDefinition checkProcessDefinitionByName(String processDefinitionName) {
        ...
        List<ProcessDefinition> processDefinition = ActivitiUtil.instance().getRepositoryService().createProcessDefinitionQuery()
                ...
                .list();
        // 判断错误并抛出
        ActivitiFunctionUtils.isWrong(CollectionUtils.isEmpty(processDefinition)).throwCustomException("流程定义" + processDefinitionName + "未找到");
        ActivitiFunctionUtils.isWrong(processDefinition.size() > 1).throwCustomException("根据给定的流程名称" + processDefinitionName + ", 查找到多个流程定义");
    ...
    public static HistoricProcessInstance checkHistoricProcessInstanceById(String processInstanceId) {
        List<HistoricProcessInstance> historicProcessInstance = ActivitiUtil.instance().getHistoryService().createHistoricProcessInstanceQuery()
                ...
                .list();
        // 判断错误并抛出
        ActivitiFunctionUtils.isWrong(CollectionUtils.isEmpty(historicProcessInstance)).throwCustomException("根据流程实例Id" + processInstanceId + "未找到历史流程实例");
        ActivitiFunctionUtils.isWrong(historicProcessInstance.size() > 1).throwCustomException("根据给定的流程实例Id" + processInstanceId + ", 查找到多个历史流程实例");
    ...
    public static ProcessInstance checkProcessInstanceById(String processInstanceId) {
        List<ProcessInstance> processInstance = ActivitiUtil.instance().getRuntimeService().createProcessInstanceQuery()
                ...
                .list();
        // 判断错误并抛出
        ActivitiFunctionUtils.isWrong(CollectionUtils.isEmpty(processInstance)).throwCustomException("根据流程实例Id" + processInstanceId + "未找到流程实例");
        ActivitiFunctionUtils.isWrong(processInstance.size() > 1).throwCustomException("根据给定的流程实例Id" + processInstanceId + ", 查找到多个流程实例");
